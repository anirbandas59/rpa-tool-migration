...
class PADGenerator:
    ...
    def generate_page(
        ...
        try:
            ...
            # Flow header (@@ConnectionString / @@Type / IMPORT / @SENSITIVE /
            # @INPUT) must be the very first content of every .robin script:
            # PAD's `@@` directives are only recognised at the top of the file,
            # and each page is packaged as its own desktop flow whose
            # <Definition> the reference solution starts with @@ConnectionString.
            # Nothing — not even a comment — may precede it.
            input_vars = []
            ...
            # Provenance banner — after the directives, before the FUNCTION.
