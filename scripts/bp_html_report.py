...
def _badge(stage_type: str, raw_type: str = None) -> str:
...
def _build_flow_chain(page_id: str, stages: list, stage_by_id: dict) -> list[dict]:
    """
    Trace the happy-path chain from Start through onsuccess.
    Returns list of {name, type, raw_type, id} dicts.
    Stops at cycles, missing targets, or cross-page jumps.
    """
    page_stages = [s for s in stages if s["page_id"] == page_id]
    start = next((s for s in page_stages if s["type"] == "Start"), None)
    if not start:
        return []
        ...
        chain.append(
            {
                "name": current["name"],
                "type": current["type"],
                "raw_type": current["raw_type"],
                "id": current["id"],
            }
        )
        next_id = current.get("onsuccess")
        if not next_id:
            break
        nxt = stage_by_id.get(next_id)
        if not nxt or nxt["page_id"] != page_id:
            # cross-page or dangling
            if nxt:
                chain.append(
                    {
                        "name": f"[{nxt['name']}]",
                        "type": nxt["type"],
                        "raw_type": nxt["raw_type"],
                        "id": nxt["id"],
                        "cross": True,
                    }
                )
...
def _render_page_section(
    ...
    # Separate flow / data / block stages
    FLOW = {
        "Start",
        "End",
        "Action",
    ...
    flow_stages = [s for s in page_stages if s["type"] in FLOW]
    data_stages = [s for s in page_stages if s["type"] in ("Data", "Collection")]
    block_stages = [s for s in page_stages if s["type"] == "Block"]
    ...
    header += f"""
      <div style="margin-bottom:14px">
        <div style="font-size:11px;font-weight:600;color:#888;text-transform:uppercase;
                    letter-spacing:.5px;margin-bottom:6px">Happy-path flow</div>
...
def generate(result: dict) -> str:
    ...
    for pid, page in _page_order(pages):
        page_sections_html += _render_page_section(
            pid, page, stages, edges, stage_by_id, pages, page_callers
        )
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python bp_html_report.py <file.xml> [output.html]")
        sys.exit(1)

    filepath = sys.argv[1]
    basename = os.path.splitext(os.path.basename(filepath))[0]
    outpath = sys.argv[2] if len(sys.argv) > 2 else f"{basename}_report.html"
