...
def generate(result: dict, graph_fn=None) -> str:
    ...
    pub_val = meta.get("artifact_type", "")
    artifact_icon = "⚙" if pub_val == "object" else "▶"
...
<div class="wrap">
  ...
  <div class="header-card">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:4px">
      <span style="font-size:22px">{artifact_icon}</span>
      ...
      <span style="background:#e8f0fe;color:#0C447C;padding:2px 8px;border-radius:10px;
                   font-size:11px;font-weight:600">{_e(meta["artifact_type"].upper())}</span>
    </div>
