...
def _parse_stage(
    stage: ET.Element, artefact_type: str, anchor_map: dict[str, str]
) -> dict[str, Any] | None:
    ...
    page_id = (
        ...
        else IMPLICIT_MAIN_KEY(artefact_type)
    )
...
def IMPLICIT_MAIN_KEY(artefact_type: str) -> str:
...
def parse_element(root_el: ET.Element, release_id: str | None = None) -> dict[str, Any]:
    ...
    for raw_s in raw_stages:
        result = _parse_stage(raw_s, artefact_type, anchor_map)
        if result is None:
            skipped_count += 1
        else:
