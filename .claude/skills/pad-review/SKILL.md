...
`run_in_background: false`) with the task ID and any context you have about what was implemented (recent git diff for the files in that task's scope is a reasonable substitute for an implementer's summary if one isn't available — check `git log`/`git diff` for those files before spawning the reviewer, so it isn't reviewing blind).

Relay the reviewer's `%` + per-dimension breakdown + gap list to the user plainly once it reports back — don't summarize away the specifics.
